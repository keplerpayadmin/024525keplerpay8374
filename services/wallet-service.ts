...
// ABI mínima para transferência de tokens ERC20
...
class WalletService {
  ...
  async getTokenBalances(walletAddress: string): Promise<TokenBalance[]> {
    try {
      ...
      for (const [symbol, tokenInfo] of Object.entries(TOKENS_BY_SYMBOL)) {
        try {
      ...
      return Object.entries(TOKENS_BY_SYMBOL).map(([symbol, tokenInfo]) => ({
        symbol: tokenInfo.symbol,
        name: tokenInfo.name,
        address: tokenInfo.address,
        balance: "0",
  ...
  async sendToken(params: SendTokenParams): Promise<SendTokenResult> {
    try {
      ...
      if (!params.tokenAddress) {
        ...
        // Determinar decimais do token (assumindo 18 para a maioria, 6 para USDC)
        const decimals =
          params.tokenAddress.toLowerCase() === "0x79A02482A880bCE3F13e09Da970dC34db4CD24d1".toLowerCase() ? 6 : 18

        // Converter amount para wei
        const amountWei = ethers.parseUnits(params.amount.toString(), decimals)

        console.log(`💰 Sending ${params.amount} tokens (${amountWei.toString()} wei) para ${params.to}`)
